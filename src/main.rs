...
use iced::{
    ...
    keyboard::{self, Key},
    widget::{
        Column, Id, column, container, image, mouse_area, operation, row, scrollable, svg, text,
        text_input,
    },
    window,
};
...
impl Launcher {
    ...
    fn update(&mut self, message: Message) -> iced::Task<Message> {
        match message {
            Message::QueryChanged(new_query) => match self.mode {
                InputMode::Command => {
    ...
    fn scroll_to_selected(&self) -> Task<Message> {
        ...
        let scrollable_height =
            CONFIG.window.height - INPUT_AREA - FOOTER_HEIGHT - PADDING_TOP * 2.0;
        ...
        if let Some(shortcut) = self.shortcut_engine.detect(&self.query) {
            self.results = vec![SearchResult {
                item: LauncherItem {
                    ...
                    subtitle: Some(
                        "Press Enter to run | Tab for command mode (slot filling)".into(),
                    ),
                    ..Default::default()
                },
                score: 100.0,
                ...
                .fold(Column::new().spacing(4), |col, (i, result)| {
                    ...
                    if i == 0 {
                        if let Some(ref calc_val) = self.calc_result {
                            ...
                            let card = container(
                                column![
                                    text(calc_val.as_str()).size(36).style(|_| text::Style {
                    ...
                    let icon_widget: Element<_> = match &result.item.icon_path {
                        Some(path) if path.ends_with(".svg") => {
                            svg(path).width(32).height(32).into()
                        }
                        Some(path) => image(path).width(32).height(32).into(),
                        None => container(text("")).width(0).height(0).into(),
                    };
                    ...
                    let item_col = column![text(&result.item.title).size(18), subtitle].spacing(2);
    ...
    fn subscription(&self) -> Subscription<Message> {
        ...
        let hotkey_sub =
            iced::time::every(std::time::Duration::from_millis(50)).map(|_| Message::PollHotkey);

        let window_sub = window::events().map(|(_, event)| match event {
